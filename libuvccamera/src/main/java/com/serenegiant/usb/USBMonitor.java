...
public final class USBMonitor {
    ...
    public USBMonitor(final Context context, final OnDeviceConnectListener listener) {
        ...
        destroyed = false;
        d(TAG, "USBMonitor mUsbManager= " + mUsbManager);
...
//                mPermissionIntent = PendingIntent.getBroadcast(context, 0, new Intent(ACTION_USB_PERMISSION), 0);
                //TODO sdk33在初始授权会失败,导致device: null,目前设置flag: 0可以解决 2023-03-23
                int flags;
    ...
    private final void processCancel(final UsbDevice device) {
        ...
        if (mOnDeviceConnectListener != null && device != null) {
    ...
    public static final String getDeviceKeyName(final UsbDevice device, final String serial, final boolean useNewAPI) {
        ...
        if (useNewAPI && BuildCheck.isAndroid5()) {
            ...
            if (TextUtils.isEmpty(serial)) {
                try {
                    UsbManager mUsbManager = (UsbManager) Utils.getApp().getSystemService(Context.USB_SERVICE);
                    if (Utils.getApp().getApplicationInfo().targetSdkVersion >= 29) {
                        if (mUsbManager.hasPermission(device)) {
                            sb.append(device.getSerialNumber());
                            sb.append("#SN, ");    // API >= 21
                        } else {
                            Log.w(TAG, "还没授权,不允许读取SerialNumber");
                        }
                    } else {
                        sb.append(device.getSerialNumber());
                        sb.append("#SN, ");   // API >= 21
                    ...
                    Log.e(TAG, "读取设备号异常: " + e.getMessage());
                }
            }
