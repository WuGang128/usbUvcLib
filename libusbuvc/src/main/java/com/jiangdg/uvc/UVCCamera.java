...
import com.jiangdg.utils.UvcLog;
...
public class UVCCamera {
    ...
    public static final int PU_AVIDEO_STD = 0x80010000;    // D16: AnaUvcLog Video Standard
    public static final int PU_AVIDEO_LOCK = 0x80020000;    // D17: AnaUvcLog Video Lock Status
    ...
    protected int mAnaUvcLogVideoStandardMin, mAnaUvcLogVideoStandardMax, mAnaUvcLogVideoStandardDef;
    protected int mAnaUvcLogVideoLockStateMin, mAnaUvcLogVideoLockStateMax, mAnaUvcLogVideoLockStateDef;
    // until here
...
//			long id_camera, int venderId, int productId, int fileDescriptor, int busNum, int devAddr, String usbfs
        ...
        if (USBMonitor.DEBUG) {
            UvcLog.i(TAG, "open camera status: " + mNativePtr + ", size: " + mSupportedSize);
        }
...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
        ...
        if (DEBUG) UvcLog.v(TAG, "close:finished");
    }
...
// 			   UvcLog.d(TAG, "setZoom:zoom=" + zoom + " ,value=" + z);
...
//================================================================================
        ...
        if (mNativePtr != 0) {
            if ((mControlSupports == 0) || (mProcSupports == 0)) {
                ...
                if (false) {
                    ...
                    UvcLog.v(TAG, String.format("Brightness:min=%d,max=%d,def=%d", mBrightnessMin, mBrightnessMax, mBrightnessDef));
                    UvcLog.v(TAG, String.format("Contrast:min=%d,max=%d,def=%d", mContrastMin, mContrastMax, mContrastDef));
                    UvcLog.v(TAG, String.format("Sharpness:min=%d,max=%d,def=%d", mSharpnessMin, mSharpnessMax, mSharpnessDef));
                    UvcLog.v(TAG, String.format("Gain:min=%d,max=%d,def=%d", mGainMin, mGainMax, mGainDef));
                    UvcLog.v(TAG, String.format("Gamma:min=%d,max=%d,def=%d", mGammaMin, mGammaMax, mGammaDef));
                    UvcLog.v(TAG, String.format("Saturation:min=%d,max=%d,def=%d", mSaturationMin, mSaturationMax, mSaturationDef));
                    UvcLog.v(TAG, String.format("Hue:min=%d,max=%d,def=%d", mHueMin, mHueMax, mHueDef));
                    UvcLog.v(TAG, String.format("Zoom:min=%d,max=%d,def=%d", mZoomMin, mZoomMax, mZoomDef));
                    UvcLog.v(TAG, String.format("WhiteBlance:min=%d,max=%d,def=%d", mWhiteBlanceMin, mWhiteBlanceMax, mWhiteBlanceDef));
                    UvcLog.v(TAG, String.format("Focus:min=%d,max=%d,def=%d", mFocusMin, mFocusMax, mFocusDef));
                }
            }
        } else {
            mControlSupports = mProcSupports = 0;
    ...
    private static final String[] SUPPORTS_PROC = {
            ...
            "D16: AnaUvcLog Video Standard",
            "D17: AnaUvcLog Video Lock Status",
            "D18: Contrast, Auto",
    ...
    private static final void dumpControls(final long controlSupports) {
        UvcLog.i(TAG, String.format("controlSupports=%x", controlSupports));
        for (int i = 0; i < SUPPORTS_CTRL.length; i++) {
            UvcLog.i(TAG, SUPPORTS_CTRL[i] + ((controlSupports & (0x1 << i)) != 0 ? "=enabled" : "=disabled"));
        }
    }

    private static final void dumpProc(final long procSupports) {
        UvcLog.i(TAG, String.format("procSupports=%x", procSupports));
        for (int i = 0; i < SUPPORTS_PROC.length; i++) {
            UvcLog.i(TAG, SUPPORTS_PROC[i] + ((procSupports & (0x1 << i)) != 0 ? "=enabled" : "=disabled"));
        }
    }
    ...
    private final String getUSBFSName(final UsbControlBlock ctrlBlock) {
        ...
        if (TextUtils.isEmpty(result)) {
            UvcLog.w(TAG, "failed to get USBFS path, try to use default path:" + name);
