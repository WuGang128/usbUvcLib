...
public final class USBMonitor {
    ...
    public USBMonitor(final Context context, final OnDeviceConnectListener listener) {
        UvcLog.v(TAG, "USBMonitor:Constructor");
        if (listener == null)
        ...
        destroyed = false;
        UvcLog.v(TAG, "USBMonitor:mUsbManager=" + mUsbManager);
    }
...
//			UvcLog.i(TAG, "unregister:");
    ...
    /**
     * output device list to UvcLogCat
     */
        ...
        if (list != null) {
            ...
            if (keys != null && keys.size() > 0) {
                ...
                for (final String key : keys) {
                    ...
                    UvcLog.i(TAG, "key=" + key + ":" + device + ":" + sb.toString());
                }
            } else {
                UvcLog.i(TAG, "no device");
            }
        } else {
            UvcLog.i(TAG, "no device");
        }
    ...
    public final boolean hasPermission(final UsbDevice device) throws IllegalStateException {
        if (destroyed) {
            UvcLog.w(TAG, "hasPermission failed, camera destroyed!");
            return false;
        }
    ...
    private boolean updatePermission(final UsbDevice device, final boolean hasPermission) {
        ...
        try {
            ...
            UvcLog.w("jiangdg", e.getLocalizedMessage());
        }
...
//		UvcLog.v(TAG, "requestPermission:device=" + device);
        boolean result = false;
        if (isRegistered()) {
            if (device != null) {
                UvcLog.i(TAG, "request permission, has permission: " + mUsbManager.hasPermission(device));
                if (mUsbManager.hasPermission(device)) {
                    ...
                    try {
                        // パーミッションがなければ要求する
                        UvcLog.i(TAG, "start request permission...");
                        mUsbManager.requestPermission(device, mPermissionIntent);
                    } catch (final Exception e) {
                        // Android5.1.xのGALAXY系でandroid.permission.sec.MDM_APP_MGMTという意味不明の例外生成するみたい
                        UvcLog.w(TAG, "request permission failed, e = " + e.getLocalizedMessage(), e);
                ...
                if (DEBUG)
                    UvcLog.w(TAG, "request permission failed, device is null?");
                processCancel(device);
                result = true;
            }
        } else {
            if (DEBUG)
                UvcLog.w(TAG, "request permission failed, not registered?");
            processCancel(device);
            result = true;
        }
        return result;
                ...
                synchronized (USBMonitor.this) {
                    ...
                    if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
                        if (device != null) {
                            ...
                            if (DEBUG)
                                UvcLog.w(TAG, "get permission success in mUsbReceiver");
                            processConnect(device);
                        }
                    } else {
                        // failed to get permission
                        if (DEBUG)
                            UvcLog.w(TAG, "get permission failed in mUsbReceiver");
                        processCancel(device);
                    }
                }
    ...
    private void processConnect(final UsbDevice device) {
        ...
        mAsyncHandler.post(() -> {
            UvcLog.v(TAG, "processConnect:device=" + device.getDeviceName());
            UsbControlBlock ctrlBlock;
            ...
            if (mOnDeviceConnectListener != null) {
                if (ctrlBlock.getConnection() == null) {
                    UvcLog.e(TAG, "processConnect: Open device failed");
...
//		UvcLog.v(TAG, "getDeviceKeyName:" + sb.toString());
        return sb.toString();
    }
    ...
    public static final class UsbControlBlock implements Cloneable {
        ...
        private UsbControlBlock(final USBMonitor monitor, final UsbDevice device) {
            UvcLog.i(TAG, "UsbControlBlock:constructor");
            mWeakMonitor = new WeakReference<USBMonitor>(monitor);
            mWeakDevice = new WeakReference<UsbDevice>(device);
            mConnection = monitor.mUsbManager.openDevice(device);
            if (mConnection == null) {
                UvcLog.w(TAG, "openDevice failed in UsbControlBlock11, wait and try again");
                try {
                    Thread.sleep(500);
            ...
            if (DEBUG) {
                if (mConnection != null) {
                    ...
                    UvcLog.i(TAG, String.format(Locale.US, "name=%s,desc=%d,busnum=%d,devnum=%d,rawDesc=", name, desc, busnum, devnum));
                } else {
                    UvcLog.e(TAG, "could not connect to device(mConnection=null) " + name);
                }
            }
        }
        ...
        private UsbControlBlock(final UsbControlBlock src) throws IllegalStateException {
            ...
            mConnection = monitor.mUsbManager.openDevice(device);
            if (mConnection == null) {
                UvcLog.w(TAG, "openDevice failed in UsbControlBlock, wait and try again");
                try {
                    Thread.sleep(500);
                } catch (InterruptedException e) {
                    e.printStackTrace();
        ...
        public synchronized void close() {
            UvcLog.i(TAG, "UsbControlBlock#close:");
