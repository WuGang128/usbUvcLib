...
//			Logger.i(TAG, "unregister:");
    ...
    /**
     * output device list to LoggerCat
     */
    ...
    public final boolean hasPermission(final UsbDevice device) throws IllegalStateException {
        if (destroyed) {
            Logger.w(TAG, "hasPermission failed, camera destroyed!");
            return false;
        }
    ...
    private boolean updatePermission(final UsbDevice device, final boolean hasPermission) {
        ...
        try {
            ...
            Logger.w("jiangdg", e.getLocalizedMessage());
        }
...
//		Logger.v(TAG, "requestPermission:device=" + device);
        ...
        if (isRegistered()) {
            if (device != null) {
                Logger.i(TAG, "request permission, has permission: " + mUsbManager.hasPermission(device));
                if (mUsbManager.hasPermission(device)) {
                    ...
                    try {
                        // パーミッションがなければ要求する
                        Logger.i(TAG, "start request permission...");
                        mUsbManager.requestPermission(device, mPermissionIntent);
                    } catch (final Exception e) {
                        // Android5.1.xのGALAXY系でandroid.permission.sec.MDM_APP_MGMTという意味不明の例外生成するみたい
                        Logger.w(TAG, "request permission failed, e = " + e.getLocalizedMessage(), e);
                ...
                if (DEBUG)
                    Logger.w(TAG, "request permission failed, device is null?");
                processCancel(device);
                result = true;
            }
        } else {
            if (DEBUG)
                Logger.w(TAG, "request permission failed, not registered?");
            processCancel(device);
            result = true;
        }
        return result;
                ...
                synchronized (USBMonitor.this) {
                    ...
                    if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
                        if (device != null) {
                            ...
                            if (DEBUG)
                                Logger.w(TAG, "get permission success in mUsbReceiver");
                            processConnect(device);
                        }
                    } else {
                        // failed to get permission
                        if (DEBUG)
                            Logger.w(TAG, "get permission failed in mUsbReceiver");
                        processCancel(device);
                    }
                }
    ...
    private void processConnect(final UsbDevice device) {
        ...
        mAsyncHandler.post(() -> {
            Logger.v(TAG, "processConnect:device=" + device.getDeviceName());
            UsbControlBlock ctrlBlock;
            ...
            if (mOnDeviceConnectListener != null) {
                if (ctrlBlock.getConnection() == null) {
                    Logger.e(TAG, "processConnect: Open device failed");
...
//		Logger.v(TAG, "getDeviceKeyName:" + sb.toString());
    ...
    public static final class UsbControlBlock implements Cloneable {
        ...
        private UsbControlBlock(final USBMonitor monitor, final UsbDevice device) {
            ...
            if (mConnection == null) {
                Logger.w(TAG, "openDevice failed in UsbControlBlock11, wait and try again");
                try {
                    Thread.sleep(500);
            ...
            if (DEBUG) {
                if (mConnection != null) {
                    ...
                    Logger.i(TAG, String.format(Locale.US, "name=%s,desc=%d,busnum=%d,devnum=%d,rawDesc=", name, desc, busnum, devnum));
                } else {
                    Logger.e(TAG, "could not connect to device(mConnection=null) " + name);
                }
            }
        }
        ...
        private UsbControlBlock(final UsbControlBlock src) throws IllegalStateException {
            ...
            mConnection = monitor.mUsbManager.openDevice(device);
            if (mConnection == null) {
                Logger.w(TAG, "openDevice failed in UsbControlBlock, wait and try again");
                try {
                    Thread.sleep(500);
                } catch (InterruptedException e) {
                    e.printStackTrace();
